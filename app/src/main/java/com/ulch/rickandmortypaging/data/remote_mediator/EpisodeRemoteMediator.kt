...
import com.ulch.rickandmortypaging.data.toEpisodeList
...
class EpisodeRemoteMediator(
    ...
    override suspend fun load(
        ...
        try {
            ...
            database.withTransaction {
                ...
                dbList?.let {
                    database.episodesDao().insertAll(it)
                }
    ...
    private suspend fun getCharacterForLastItem(state: PagingState<Int, EpisodeEntity>): EpisodeEntity? {
        return state.pages.lastOrNull { it.data.isNotEmpty() }?.data?.lastOrNull()
            ?.let { character ->
                database.episodesDao().remoteId(character.id)
            }
    }

    private suspend fun getCharacterForFirstItem(state: PagingState<Int, EpisodeEntity>): EpisodeEntity? {
    ...
    private suspend fun getCharacterClosestToCurrentPosition(state: PagingState<Int, EpisodeEntity>): EpisodeEntity? {
        return state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.id?.let { repoId ->
                database.episodesDao().remoteId(repoId)
            }
        }
    }
